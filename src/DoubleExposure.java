...
public class DoubleExposure
{
    public static void main(String[] args){
        ...
        if(image1.getImageWidth() > image2.getImageWidth()){
            canvasWidth = image1.getImageWidth();
        }
        else{
            canvasWidth = image2.getImageWidth();
        }
        if(image1.getImageHeight() > image2.getImageHeight()){
            canvasHeight = image1.getImageHeight();
        }
        else{
            canvasHeight = image2.getImageHeight();
        }
        ...
        for(int i = 0; i < canvas.getImageHeight(); i++){
            for (int j = 0; j < canvas.getImageWidth(); j++){
                ...
                //initialize pixel and set it on the canvas
